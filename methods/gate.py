...
class Method:  # pylint: disable=E1101,R0903,W0201
    ...
    def usage_gate_acquire(self, project_id, user_id, estimate_micro, moment):  # pylint: disable=R0914
        ...
        #
        try:
            self.usage_gate_prime_live(project_key_name, project_key(project_id, moment))
            #
            if member_key_name:
                self.usage_gate_prime_live(
    ...
    @web.method()
    def usage_gate_prime_live(self, hash_key, counter_key):
        """Prime, and prime again when a key marked primed has since been evicted.

        _primed remembers only the key name, so without this the gate would reserve against a
        zero for the rest of the window while the spend sits in Postgres.
        """
